...
(function (root) {
  ...
  function markup(r, opts) {
    ...
    html +=
        ...
        '<div class="callout">This is a suggested budget and allocation, modeled from the market. In a ' +
          '<a href="' + esc(r.consult_url) + '" target="_blank" rel="noopener">free consultation</a>, ' +
          "Smart 1 can tailor a budget that works for your resort.</div>" +
      "</section>";

    /* ------------------------------------- good / better / best investment */
    var pt = r.pricing_tiers || {};
    if (pt.tiers && pt.tiers.length) {
      html +=
        '<section class="block">' +
          '<div class="block-eyebrow">Section 4a — Investment levels for this market</div>' +
          "<h3>Good, better, best — sized to your market</h3>" +
          '<p class="lede">' + esc(pt.basis) + "</p>" +
          '<div class="tiers">' +
          pt.tiers.map(function (t) {
            return '<div class="tier' + (t.recommended ? " is-rec" : "") + '">' +
              (t.recommended ? '<div class="tier-flag">Recommended for this market</div>' : "") +
              '<div class="tier-name">' + esc(t.label) + "</div>" +
              '<div class="tier-stance">' + esc(t.stance) + "</div>" +
              '<div class="tier-price">' + money(t.monthly) + '<span>/mo</span></div>' +
              '<div class="tier-season">≈ ' + money(t.season_estimate) + " per season · " + int(t.monthly_impressions) + " impressions/mo</div>" +
              '<p class="tier-desc">' + esc(t.description) + "</p>" +
              '<div class="tier-mix">' + (t.channels || []).map(function (c) {
                return '<div class="tier-mix-row"><span>' + esc(c.channel) + "</span><b>" + money(c.budget) + "</b></div>";
              }).join("") + "</div>" +
            "</div>";
          }).join("") +
          "</div>" +
          (pt.entered_vs_tiers ? '<p class="lede" style="margin-top:14px"><strong>' + esc(pt.entered_vs_tiers) + "</strong></p>" : "") +
          '<div class="callout">' + esc(pt.disclaimer) + ' <a href="' + esc(pt.consult_url) + '" target="_blank" rel="noopener">Schedule a consultation →</a></div>' +
        "</section>";
    }

    /* ------------------------------------------- against traditional media */
    var vt = r.vs_traditional || {};
    if (vt.rows && vt.rows.length) {
      html +=
        '<section class="block">' +
          '<div class="block-eyebrow">Section 4b — Against traditional advertising</div>' +
          "<h3>" + esc(vt.headline) + "</h3>" +
          "<table class='vs'><thead><tr><th></th><th>Traditional buy</th><th>This plan</th></tr></thead><tbody>" +
          vt.rows.map(function (row) {
            return "<tr><td><strong>" + esc(row.dimension) + "</strong></td>" +
              "<td>" + esc(row.traditional) + "</td>" +
              '<td class="vs-us">' + esc(row.this_plan) + "</td></tr>";
          }).join("") +
          "</tbody></table>" +
          '<div class="deep-panel"><div class="deep-label">The season in one line</div>' +
            '<div class="deep-big">' + esc(vt.season_frame) + "</div></div>" +
          '<p class="methodology">' + esc(vt.caveat) + "</p>" +
        "</section>";
    }
    ...
    html +=
      '<section class="block cta-block">' +
        '<div class="cta-panel">' +
          "<h3>Have questions about these results?</h3>" +
          "<p>A 15–30 minute review can turn this into a budget and launch plan built for your resort — " +
            "season timing · trigger setup · audience counts · creative needs · a budget that fits.</p>" +
          '<a class="cta-btn" href="' + esc(r.consult_url) + '" target="_blank" rel="noopener">Schedule a ski marketing consult</a>' +
        "</div>" +
      "</section>";
